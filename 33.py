...
def load_vacancies_into_database():
    ...
    try:
        with connection.cursor() as cursor:
            # Create job_vacancies table if not exists
            create_table_query = """
            ...
            while True:
                ...
                if response.status_code == 200:
                    ...
                    for vacancy in vacancies:
                        vacancy_data = {
                            'title': clean_text(
                                vacancy.find('a', class_='vacancy-card__title-link').text),
                            'company': clean_text(
                                vacancy.find('div', class_='vacancy-card__company-title').text),
                            'location': clean_text(
                                vacancy.find('div', class_='vacancy-card__meta').text),
                            'requirements': clean_text(
                                vacancy.find('div', class_='vacancy-card__skills').text) if vacancy.find('div', class_='vacancy-card__skills') else '',
                            'salary': clean_text(
                                vacancy.find('div', class_='vacancy-card__salary').text),
...
def search_vacancies():
    if request.method == 'POST':
        load_vacancies_into_database()  # Загрузить вакансии в базу данных
        ...
        try:
            with connection.cursor() as cursor:
                # Получаем результаты из базы данных согласно поисковому запросу
                search_query = """
                ...
                # Получаем количество всех вакансий из базы данных
